          ...
          <nav>
            <ul className="flex gap-6 text-white font-bold text-sm md:text-base">
              <li onClick={onHomeClick}
                className="hover:text-blue-300 cursor-pointer transition-all hover:scale-105 active:scale-95"> الرئيسية</li>
              <li onClick={() => setShowAbout(true)}
                className="hover:text-blue-300 cursor-pointer transition-all hover:scale-105 active:scale-95">عن التطبيق</li>
            </ul>
          </nav>
        </div>
      </header>
      ...
      {showAbout && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-gradient-to-br from-slate-800 to-slate-900 border border-white/20 p-8 rounded-[2rem] max-w-md w-full shadow-2xl relative">
            <button onClick={() => setShowAbout(false)}className="absolute top-4 left-4 text-white/50 hover:text-white text-2xl">✕</button>
            ...
            <button onClick={() => setShowAbout(false)}className="w-full py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-bold transition-colors">فهمت ذلك</button>
          </div>
        </div>
      )}
    </>
